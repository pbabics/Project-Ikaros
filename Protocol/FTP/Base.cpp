...
void processRecieve(in_addr /* address */, int fd, char* data)
{
    ...
    switch (processCommand)
    {
        ...
        case FTP_LOGIN_PASS:
        {
            ...
            protoLog->outDebug("FTP:: User: %s  pass: '%s'", session.username.c_str(), session.password.c_str());
        ...
        case FTP_MAKE_DIR:
        {
            ...
            string address = '.' + session.actualDir + dirname;
        ...
        case FTP_REMOVE_DIR:
        {
            ...
            string address = '.' + session.actualDir + filename;
